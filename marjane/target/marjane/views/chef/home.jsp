                ...
                <div class="grid grid-cols-3 gap-8 items-center">
                    <div class="col-span-2">
                        <nav class="flex space-x-4">
                            <a href="adminDashboardHome" class="text-white text-sm font-medium rounded-md bg-white bg-opacity-0 px-3 py-2 hover:bg-opacity-10" aria-current="page"> Actual Promotions </a>
                ...
                <div class="rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 bg-white divide-y divide-gray-200">
                    <div class="pt-3 pb-2">
                        ...
                        <div class="mt-3 px-2 space-y-1">
                            <a href="/adminDashboardHome" class="block rounded-md px-3 py-2 text-base text-gray-900 font-medium hover:bg-gray-100 hover:text-gray-800">Actual Promotion </a>
            ...
            <div class="grid grid-cols-1 gap-4 items-start lg:grid-cols-3 lg:gap-8">
                                            ...
                                            <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                                                <table class="min-w-full divide-y divide-gray-200">
                                                    ...
                                                    <c:forEach items="${promotions}" var="promotion">
                                                        <tr class="bg-white">
                                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"><c:out value="${promotion.id}"/></td>
                                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"><c:out value="${promotion.date}"/></td>
                                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"><c:out value="${promotion.endDate}"/></td>
                                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"><c:out value="${promotion.promo}"/></td>
                                                            <!--<c:out value="${promotion.status}"/>-->
                                                            <c:choose>
                                                                <c:when test="${promotion.status==0}">
                                                                    <td class="px-4 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                                        <span class="text-red-600 hover:underline  underline-offset-1 hover:cursor-pointer"  onclick="myFunction(1,${promotion.id})">Refuse</span>
                                                                    </td>
                                                                    <td class="px-4 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                                        <span class="text-green-600 hover:underline  underline-offset-1 hover:cursor-pointer" onclick="myFunction(2,${promotion.id})">Accept</span>
                                                                    </td>
                                                                </c:when>
                                                                <c:when test="${promotion.status==1}">
                                                                <td colspan="2" class="px-4 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800"> Refused </span>
                                                                </td>
                                                                </c:when>
                                                                <c:otherwise>
                                                                    <td colspan="2" class="px-4 py-4  whitespace-nowrap text-right text-sm font-medium">
                                                                        <span class="inline-flex items-center mx-auto px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800"> Accepted </span>
                                                                    </td>
                                                                </c:otherwise>
                                                            </c:choose>
                                                        </tr>
                ...
                <div class="grid grid-cols-1 gap-4">
                    <section aria-labelledby="section-2-title">
                        ...
                        <div class="rounded-lg bg-white overflow-hidden shadow">
                            <div class="p-6">
                                <h1>Comment for treated promotions </h1>
                                <form action="action" method="post" id="commentForm">
                                    <input type="hidden" name="action" id="inputAction" />
                                    <input type="hidden" name="promoId" id="inputPromoId"/>
                                    <div class="mt-1">
                                        <textarea rows="6" name="comment" id="commentText"  class="shadow-sm border-2 p-3 block w-full sm:text-sm border-blue-900 rounded-md" placeholder="Write a comment before accepting ot refusing a promotion" required></textarea>
                                    </div>
                                    <span id="message" class="text-sm mt-3 "></span>
                                    <input type="submit" id="comment"
                                          class=" mt-4 cursor-not-allowed inline-flex items-center px-7 m-auto mt-3 py-2 border border-transparent text-base font-medium rounded-md text-indigo-700 bg-indigo-100 hover:bg-indigo-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" value="Submit"/>
                                </form>
                            </div>
                        </div>
                    </section>
...
<script>
    ...
    let inputPromoId = document.getElementById("inputPromoId")
    ...
    myForm.addEventListener("submit",(ev)=>{
        ...
        console.log("promo input : "+inputPromoId.value)
        console.log("action input : "+inputAction.value)
    })
     */

    function clear(id){
        comment.classList.remove("text-indigo-700")
        comment.classList.remove("bg-indigo-100")
        comment.classList.remove("hover:bg-indigo-200")
        comment.classList.remove("focus:ring-indigo-500")
        message.classList.remove("text-indigo-700")
        switch (id){
            case 1:
                comment.classList.remove("text-green-700")
                comment.classList.remove("bg-green-100")
                comment.classList.remove("hover:bg-green-200")
                comment.classList.remove("focus:ring-green-500")
                message.classList.remove("text-green-700")
            case 2:
                comment.classList.remove("text-red-700")
                comment.classList.remove("bg-red-100")
                comment.classList.remove("hover:bg-red-200")
                comment.classList.remove("focus:ring-red-500")
    ...
    function myFunction(action, id) {
        ...
        inputPromoId.value = id

        switch (action){
            case 1:
                clear(action)
                comment.classList.remove("cursor-not-allowed")
                comment.classList.add("text-red-700")
                comment.classList.add("bg-red-100")
                comment.classList.add("hover:bg-red-200")
                comment.classList.add("focus:ring-red-500")
                comment.value = "Refuse"
                message.classList.add("text-red-700")
                message.innerHTML = "Tell the admin why you refused the promotion"
                break;
            case 2:
                clear(action)
                comment.classList.remove("cursor-not-allowed")
                comment.classList.add("text-green-700")
                comment.classList.add("bg-green-100")
                comment.classList.add("hover:bg-green-200")
                comment.classList.add("focus:ring-green-500")
                message.classList.add("text-green-700")
                comment.value = "Accept"
                message.innerHTML = "Tell the admin why you accepted the promotion"
                break;
        }
    };
</script>
